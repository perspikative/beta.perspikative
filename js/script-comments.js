...
// + Système complet de restrictions : limite quotidienne, cooldown,
//   longueur, anti-spam, liens interdits, mots interdits, nettoyage,
//   signalements, compteur de signalements, masquage automatique.
// ──────────────────────────────────────────────────────────────────────────────
...
(function () {
  ...
  var MAX_LEN           = 750;
  var WARN_THRESHOLD    = 50; // le compteur devient rouge sous ce seuil restant
  ...
  // Élément compteur de caractères, créé dynamiquement s'il n'existe pas déjà
  var charCounterEl = document.getElementById('lb-comment-char-counter');
  if (!charCounterEl && inputWrap) {
    charCounterEl = document.createElement('span');
    charCounterEl.id = 'lb-comment-char-counter';
    charCounterEl.className = 'lb-comment-char-counter';
    inputWrap.appendChild(charCounterEl);
  }
  ...
  function updateCharCounter() {
    if (!charCounterEl || !textarea) return;
    var len = textarea.value.length;
    charCounterEl.textContent = len + ' / ' + MAX_LEN;
    charCounterEl.classList.toggle('lb-char-counter-warn', (MAX_LEN - len) < WARN_THRESHOLD);
  }
  ...
  function sendComment() {
    ...
    // ── 2. Vérification cooldown + limite quotidienne ─────────────────────
    rl.canSendComment(user.uid).then(function (check) {
      if (!check.allowed) {
        if (check.reason === 'daily_limit') {
          ...
          showFormError('Merci de patienter avant de publier un nouveau commentaire.');
          textarea.disabled = false;
          rl.startCooldownUI(sendBtn, check.remainingMs, 'Publier');
        }
        return;
      }
      ...
      fire.addDoc(colRef, {
        ...
        // Démarre le cooldown visuel de 30s sur le bouton
        rl.startCooldownUI(sendBtn, rl.COOLDOWN_MS, 'Publier');
      }).catch(function (err) {
  ...
  document.addEventListener('prspk:auth-ready', function (e) {
    ...
    // Re-render pour mettre à jour les boutons supprimer / signaler
